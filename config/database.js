...
const getSSLOptions = (env) => {
  // Only enable SSL if explicitly requested for this environment
  let isEnabled = false;
  ...
  if (env === 'test') {
    ...
    // Development MUST be explicit to avoid breaking local setups
...
export default new Proxy(exportedConfig, {
  get: (target, prop) => {
